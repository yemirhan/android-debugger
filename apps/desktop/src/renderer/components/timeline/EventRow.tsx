...
import type { TimelineEvent, TimelineEventType } from '@android-debugger/shared';
...
export function EventRow({ event, isSelected, onClick }: EventRowProps) {
  const severityClass = event.severity ? severityColors[event.severity] : severityColors.info;
  const typeColor = typeColors[event.type];

  return (
    <div
      onClick={onClick}
      className={`flex items-start gap-3 px-3 py-2 border-l-2 cursor-pointer transition-colors ${severityClass} ${
        isSelected
          ? 'bg-accent-muted'
    ...
    >
      ...
      <span className={`flex-shrink-0 pt-0.5 ${typeColor}`}>
        {typeIcons[event.type]}
      </span>
      <div className="flex-1 min-w-0">
        <div className="text-sm text-text-primary truncate">{event.title}</div>
        {event.subtitle && (
          <div className="text-xs text-text-muted truncate">{event.subtitle}</div>
        )}
      </div>
      <span className={`text-xs px-1.5 py-0.5 rounded-full bg-surface-elevated ${typeColor} uppercase font-medium`}>
